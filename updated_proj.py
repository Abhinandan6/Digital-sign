...
def sign_document(file_path):
    try:
        ...
        # Embed QR code in the signed PDF document
...
def embed_qr_code(signed_file_path, qr_image):
    # Create a new PDF file with embedded QR code
    output_file_path = signed_file_path[:-4] + "_qr_embedded.pdf"

    # Open the existing signed PDF document
    existing_pdf = PdfReader(signed_file_path)
    output_pdf = PdfWriter()

    # Create a canvas for drawing on the existing pages
    c = canvas.Canvas(output_file_path, pagesize=existing_pdf.pages[0].mediaBox)

    # Draw the QR code image at the specified position (e.g., bottom-left corner)
    c.drawInlineImage(qr_image, x=50, y=50, width=100, height=100)

    # Add the modified pages to the output PDF
    for page in existing_pdf.pages:
        c.setPageSize((page.mediaBox.getWidth(), page.mediaBox.getHeight()))
        c.showPage()
        output_pdf.add_page(page)

    # Save the modified PDF with embedded QR code
